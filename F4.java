...
interface IAttackSeaForce { //可对海攻击
    default void attack(INavyForce x) {
        System.out.print("可以攻击海军");
    }
}
...
class HeavyTank extends Tank  implements IAttackAirLandForce {//重型坦克类
    ...
    public void attack(Arms x){//重型坦克可对空、对地攻击
        ...
        if(x instanceof Bomber) System.out.print("用导弹攻击轰炸机");
        if(x instanceof AmericanTank) System.out.print("用重炮攻击美式坦克");
        //其他兵种攻击方式同理
    }
}
...
class AmphibiousTank extends Tank implements IAttackLandForce, IAttackAirForce, IAttackSeaForce {
    public AmphibiousTank(){super("水陆两栖坦克");}
    public void attack(ILandForce x){ System.out.print("水陆两栖坦克"); }
    public void attack(IAirForce x){ System.out.print("水陆两栖坦克"); }
    public void attack(INavyForce x){ System.out.print("水陆两栖坦克"); }
    public void attack(Arms x){
        System.out.print("\n"+attackInfo(x)+"：");
        if(x instanceof ILandForce) attack((ILandForce)x);
        else if(x instanceof IAirForce) attack((IAirForce)x);
        else if(x instanceof INavyForce) attack((INavyForce)x);
        else System.out.print("不能攻击！");
        if(x instanceof Soldier) System.out.print("用炮火攻击士兵");
        if(x instanceof Bomber) System.out.print("用导弹攻击轰炸机");
        if(x instanceof Destroyer) System.out.print("用鱼雷攻击驱逐舰");
        // 其他兵种攻击方式同理
    }
}
...
abstract class NavalVehicle extends Arms implements INavyForce {
    public NavalVehicle(String n) {super(n); }
}
class Destroyer extends NavalVehicle implements IAttackSeaForce, IAttackLandForce {
    public Destroyer() {    super(" 驱逐舰 ");  }
    public void attack(ILandForce x) {
        System.out.print("驱逐舰"); }
    public void attack(IAirForce x) {
        System.out.print("驱逐舰");}
    public void attack(INavyForce x) {
        System.out.print("驱逐舰"); }
 public void attack(Arms x) {
        System.out.print("\n" + attackInfo(x) + "：");
        if (x instanceof ILandForce)
            attack((ILandForce) x);
        else if (x instanceof IAirForce)
            attack((IAirForce) x);
        else if (x instanceof INavyForce)
            attack((INavyForce) x);
        else
            System.out.print("不能攻击！");
        if (x instanceof Soldier)
            System.out.print("用炮弹攻击士兵");
        if (x instanceof LightTank)
            System.out.print("用炮弹攻击轻型坦克");
        if (x instanceof Bomber)
            System.out.print("用导弹攻击轰炸机");
        // 其他兵种攻击方式同理
    }
 }
//美式坦克类
class AmericanTank extends Tank implements ILandForce {
 public AmericanTank() {
     super("美式坦克");
 }

 public void attack(ILandForce x) {
     System.out.print("美式坦克");
 }
 ...
 public void attack(Arms x) {
     ...
     if (x instanceof ILandForce)
         ...
         System.out.print("不能攻击！");
     if (x instanceof Soldier)
         System.out.print("用火箭弹攻击士兵");
     if (x instanceof LightTank)
         System.out.print("用火箭弹攻击轻型坦克");
     // 其他兵种攻击方式同理
 }
}

//苏联坦克类
class SovietTank extends Tank implements ILandForce {
 public SovietTank() {
     super("苏联坦克");
 }

 public void attack(ILandForce x) {
     System.out.print("苏联坦克");
 }
...
//尤里坦克类
class YuriTank extends Tank implements ILandForce {
 public YuriTank() {
     super("尤里坦克");
 }

 public void attack(ILandForce x) {
     System.out.print("尤里坦克");
 }
 ...
 public void attack(Arms x) {
     ...
     if (x instanceof ILandForce)
         ...
         System.out.print("不能攻击！");
     if (x instanceof Soldier)
         System.out.print("用火焰喷射器攻击士兵");
     if (x instanceof LightTank)
         System.out.print("用火焰喷射器攻击轻型坦克");
     // 其他兵种攻击方式同理
 }
}
...
//国家类
class Country {
 public static Tank produceTank(String country) {
     if (country.equals("美国")) {
         return new AmericanTank();
     } else if (country.equals("苏联")) {
         return new SovietTank();
     } else if (country.equals("尤里")) {
         return new YuriTank();
     } else {
         throw new IllegalArgumentException("不支持的国家");
     }
 }
}
class App {
    public static void main(String[] args) {
        Arms[] a = { new HeavyTank(), new HeavyTank(), new LightTank(), new LightTank(), new Bomber(),
                new FlyingSoldier(), new FlyingSoldier(), new Infantry(), new Infantry(), new Destroyer(), new Destroyer(),
                new AmphibiousTank(), new AmphibiousTank() };
        Arms[] b = { Country.produceTank("美国"), new Infantry(), new Infantry(), new Helicopter(), new Infantry(),
                new Helicopter(), new LightTank(), new LightTank(), Country.produceTank("美国"), new Infantry(),
                new LightTank(), new Infantry(), new Destroyer() };
